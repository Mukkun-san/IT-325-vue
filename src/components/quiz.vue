      ...
      <v-card width="1000" class="mt-6 mx-16">
        <v-stepper v-if="ready" v-model="e1" :vertical="false">
          <template v-for="Qs in NewQuiz">
            <v-stepper-content :key="`${Qs.Question + Qs.n}-c`" :step="Qs.n">
              <h3>
                <b>{{ Qs.n }}-</b>
                {{ Qs.Question }}:
              </h3>
    ...
    calcGrade() {
      ...
      axios
        .post(
          API_URL + "/grades",
          {
            s_id: JSON.parse(window.localStorage.getItem("student"))._id,
            name: this.name,
            email: this.email,
            ...
            score: this.Grade,
            spreadsheetId: JSON.parse(window.localStorage.getItem("student"))
              .sheetId,
          },
          {
            headers: {
